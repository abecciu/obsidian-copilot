...
export class KeychainService {
  ...
  deleteSecretById(keychainId: string): void {
    this.storage.deleteSecret(keychainId);
  }
   ...
   *
   * Reason: uses `deleteSecret()` to fully remove entries instead of
   * tombstoning with empty strings, so the user's keychain viewer stays clean.
