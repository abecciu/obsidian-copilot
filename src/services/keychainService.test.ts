...
describe("forgetAllSecrets", () => {
  ...
  it("propagates keychain delete failures and leaves disk/memory untouched so the user can retry", async () => {
    const secretStorage = makeSecretStorage();
    const service = KeychainService.getInstance(makeApp({ secretStorage }));
    const vaultId = service.getVaultId();
    ...
    // Reason: already-deleted entries stay deleted; disk and memory are not
    // stripped on partial failure — the user can retry.
    expect(secretStorage.deleteSecret).toHaveBeenCalledWith(idA);
    // Reason: disk and memory are not stripped on partial failure — the user can retry.
