...
import { hasEncryptionPrefix, getDecryptedKey } from "@/encryptionService";
...
export async function clearDiskSecrets(
  ...
  await runPersistenceTransaction(async () => {
    ...
    // Reason: defense-in-depth — re-check the full safety set in the service
    // layer instead of relying on the UI gating. A stale modal render or a
    // future caller that forgets `canClearDiskSecrets()` could otherwise wipe
    // the only remaining copy of an undecryptable secret.
    if (!canClearDiskSecrets(current)) {
      throw new Error(
        "Cannot clear disk secrets: keychain is unavailable, the last keychain save " +
          "did not complete safely, or there are no secrets left to clear. " +
          "Retry saving settings, then try again."
      );
    }
 ...
 * - Always write secrets to keychain FIRST
 * - If _diskSecretsCleared → strip secrets from data.json
 * - Else → keep secrets in data.json (migration window for multi-device sync)
 *
 * When keychain is NOT available:
 * - Save plaintext to data.json (fallback only)
 *
 * The skip-undecryptable corner case preserves entries whose memory value is
 * still enc_* (the only remaining copy of that secret).
 */
...
export async function loadSettingsWithKeychain(
  ...
  const hadTransitionState =
    typeof rawRec._keychainMigratedAt === "string" || settingsRec._migrationModalDismissed === true;

  const { settings: hydrated, hadFailures } = await keychain.backfillAndHydrate(settings);
  backfillHadFailures = hadFailures;

  const rec = hydrated as unknown as Record<string, unknown>;

  // Reason: vaults that have never stored secrets on disk and have no migration
  // markers should go straight to keychain-only mode. Without this, their first
  // API key save falls into the transition path and writes plaintext to data.json.
  // Guard: `rawRec._diskSecretsCleared === undefined` ensures we only promote
  // vaults that have NEVER been through a persist cycle with secrets — existing
  // users who cleared their keys will have `_diskSecretsCleared` set to true/false.
  if (
    !diskHadSecrets &&
    !hadTransitionState &&
    rawRec._diskSecretsCleared === undefined
  ) {
    rec._diskSecretsCleared = true;
  }

  // Record migration timestamp on first load that observes a healthy keychain
  // alongside disk-resident secrets. Reason (Major-2): the previous
  // `backfilledAny` gate missed the "keychain already has every secret +
  // disk still has stale copies" case, so the 7-day window never started.
  // Broaden to: any load where the keychain is safe (no failures) AND disk
  // still has secrets AND no timestamp has been recorded yet.
  const stampOnLoad = !hadFailures && diskHasSecrets && typeof rec._keychainMigratedAt !== "string";
  if (stampOnLoad) {
...
async function restoreKeychainFromSettings(
  ...
  for (const id of keychainIdsToDelete) {
    try {
      ...
      logWarn(`Failed to delete keychain entry "${id}" during rollback.`, error);
    }
  }
}
...
function resetDiskSecretFallbackFlags(settings: CopilotSettings): void {
  if (settings === getSettings()) {
    ...
    setSettings({
      ...
      //
      // Design decision: resetting the timer is intentional. This only
      // happens when secrets are RE-INTRODUCED to data.json (e.g. user
      // entered a new key on a no-keychain device). Without resetting,
      // a Day-5 write-back would be auto-cleared on Day-8 (only 3 days
      // of buffer instead of 7). "Never clears" only happens if secrets
      // keep being re-introduced — which is itself a new fallback cycle
      // that deserves a fresh window.
...
/**
 * Strip keychain-managed secrets from `source`, but preserve any field whose
 * memory value is still an encrypted-looking string.
 *
 * Reason: hydrate writes plaintext on successful decrypt and leaves the
 * original `enc_*` value on failure, so any sensitive field still matching
 * `hasEncryptionPrefix()` is the only remaining copy of a secret that the
 * keychain refused. Stripping it would lose the secret permanently.
 */
function stripSecretsPreservingFailed(source: CopilotSettings): CopilotSettings {
  const stripped = stripKeychainFields(source);
  ...
  for (const key of Object.keys(sourceRec)) {
    if (!isSecretKey(key)) continue;
    const value = sourceRec[key];
    if (typeof value === "string" && value.length > 0 && hasEncryptionPrefix(value)) {
      strippedRec[key] = value;
    }
  }
  for (const listKey of ["activeModels", "activeEmbeddingModels"] as const) {
    const sourceModels = source[listKey] ?? [];
    const strippedModels = stripped[listKey] ?? [];
    for (let i = 0; i < sourceModels.length; i++) {
      const orig = sourceModels[i] as unknown as Record<string, unknown>;
      const target = strippedModels[i] as unknown as Record<string, unknown>;
      for (const field of MODEL_SECRET_FIELDS) {
        const v = orig[field];
        if (typeof v === "string" && v.length > 0 && hasEncryptionPrefix(v)) {
          target[field] = v;
        }
      }
    }
...
async function doPersist(
  ...
  try {
    ...
    // Step 2: Build the data.json payload.
    // Three branches:
    // A) skippedUndecryptableSecret → preserve failed enc_* fields, strip rest
    // B) _diskSecretsCleared=true → strip all secrets (user chose Remove / fresh install)
    // C) migration window → keep secrets in data.json for multi-device sync
    const cleanedRec = cleaned as unknown as Record<string, unknown>;
    const diskSecretsCleared =
      (settings as unknown as Record<string, unknown>)._diskSecretsCleared === true;
    const stampedAt = new Date().toISOString();
    const stampNeeded =
      !skippedUndecryptableSecret &&
      typeof cleanedRec._keychainMigratedAt !== "string" &&
      diskHasSecrets;

    let dataToSave: CopilotSettings;
    if (skippedUndecryptableSecret) {
      // Branch A: some legacy enc_* values could not be decrypted.
      logWarn(
        "Some legacy encrypted secrets could not be decrypted for keychain write. " +
          "Preserving their on-disk ciphertext."
      );
      if (diskSecretsCleared) {
        // User already confirmed removal — strip everything except failed fields.
        dataToSave = stripSecretsPreservingFailed(cleaned);
      } else {
        // Migration window — keep ALL disk secrets intact for multi-device sync.
        dataToSave = { ...cleaned };
      }
      const dataRec = dataToSave as unknown as Record<string, unknown>;
      dataRec._diskSecretsCleared = false;
      delete dataRec._keychainMigratedAt;
      delete dataRec._migrationModalDismissed;
    } else if (diskSecretsCleared) {
      // Branch B: user confirmed (Remove now / fresh install) → strip secrets.
      dataToSave = stripKeychainFields(cleaned);
      const dataRec = dataToSave as unknown as Record<string, unknown>;
      dataRec._diskSecretsCleared = true;
      if (stampNeeded) {
        dataRec._keychainMigratedAt = stampedAt;
      }
    } else {
      // Branch C: migration window — keep current secret values in data.json
      // so other devices can still read from synced data.json until the user
      // explicitly clears or the 7-day auto-clear fires.
      dataToSave = { ...cleaned };
      if (stampNeeded) {
        (dataToSave as unknown as Record<string, unknown>)._keychainMigratedAt = stampedAt;
      }
    }

    // Step 3: Write to disk BEFORE mutating the in-memory Jotai state.
    // Reason (R2-Critical-1): if saveData() fails, the memory flags must not
    // claim "cleared" — that would leave the UI / canClearDiskSecrets() /
    // shouldShowMigrationModal() with a state the disk does not back.
    await saveData(dataToSave);
    refreshDiskHasSecrets(dataToSave);
    // Reason: lastPersistedSettings is the keychain-diff baseline, so it must
    // reflect what is in KEYCHAIN (the unstripped in-memory state), not the
    // secret-stripped data.json payload.
    lastPersistedSettings = structuredClone(settings);

    // Step 4: Sync the in-memory flags now that disk is consistent.
    if (skippedUndecryptableSecret) {
      // Reason: a failed migration moved disk back to "still has enc_*", so
      // clear the in-memory "cleared" claim if it was set by a prior save.
      const memoryRec = settings as unknown as Record<string, unknown>;
      if (memoryRec._diskSecretsCleared === true) {
        resetDiskSecretFallbackFlags(settings);
      }
    } else if (stampNeeded) {
      // Reason: sync the stamp to in-memory state (covers both Branch B and C).
      suppressNextPersistOnce();
      setSettings({ _keychainMigratedAt: stampedAt } as Partial<CopilotSettings>);
    }
